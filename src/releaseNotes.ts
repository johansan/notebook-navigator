...
const RELEASE_NOTES: ReleaseNote[] = [
    {
        ...
        showOnUpdate: true,
        new: [
            'New setting: File display > ==Show tasks==. Notes containing tasks now show a task icon, progress bar, and completed count, such as `5/7`, on the same line as the date and parent folder! Enabled by default. Colors and font weights for unfinished and completed tasks can be changed with Style Settings (e.g. green bold color for all finished tasks).',
            'You can now use ==frontmatter properties for default sorting and grouping==: each property listed in ==Sort and grouping properties== appears as an option in the ==Default sort order== and ==Default grouping== settings.',
            'You can now start a search term with a ==double quote to match it literally against note names and aliases== instead of being read as a filter: `"#work"` finds notes with `#work` in the name instead of filtering on the tag. Use `-"term"` to exclude matches.',
            'You can now temporarily ==show the root folder if it is hidden==: the `Show hidden items` toolbar button reveals the root folder dimmed at the top of the tree, and the `Search whole vault` command works while it is shown.',
            ...
            'New options in the file context menu: `Copy note link` (`[[link]]`), `Copy note link as footnote` (`^[[[link]]]`), and `Copy note embed` (`![[link]]`) in the new `Copy` submenu.'
        ],
        changed: [
